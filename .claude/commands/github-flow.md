...
- **目標 base branch**（預設 `develop`）

### Step 2：開 GitHub Issue
使用 GitHub API 建立 issue，取得 issue 編號。

```bash
gh issue create --repo benchen149/k3s-istio-sandbox \
  --title "$TITLE" \
  --body "$BODY"
```
...
```bash
gh pr create \
  --repo benchen149/k3s-istio-sandbox \
  --base $BASE_BRANCH \
...
- **絕對不可刪除** `develop`、`main` 等長期 branch，只刪除格式為 `{issue-number}-$SLUG` 的 feature branch

```bash
gh pr merge {pr-number} --repo benchen149/k3s-istio-sandbox --merge --delete-branch
git checkout $BASE_BRANCH
git pull origin $BASE_BRANCH
...
> **原因**：GitHub 的 `Closes #X` 只在 PR merge 進 default branch（`main`）時觸發，feature branch → `develop` 不會自動關閉 issue。
...
### 關閉 Issue
```bash
gh issue close {number} --repo benchen149/k3s-istio-sandbox
```

### 關閉 PR
```bash
gh pr close {number} --repo benchen149/k3s-istio-sandbox
```
...
- base branch 預設為 `develop`，若要 merge 到 `main` 請在 Step 1 指定
- `develop`、`main` 等長期 branch 不在自動刪除範圍內
